...
namespace Netfuser.Core.Impl.Embedder.Compression.Lzma
{
    internal class Encoder
    {
        ...
        public Encoder(EncoderOptions options)
        {
            ...
            var numFastBytes = options.NumFastBytes;
            if (numFastBytes < 5 || numFastBytes > Base.kMatchMaxLen)
                throw new ArgumentException(nameof(options.NumFastBytes));
            _numFastBytes = (uint)numFastBytes;
            /*
    Int32 maximize = options.Algorithm;
    _fastMode = (maximize == 0);
    _maxMode = (maximize >= 2);
    */

            EMatchFinderType matchFinderIndexPrev = _matchFinderType;
            _matchFinderType = options.MatchFinder;
            ...
            var dictionarySize = options.DictionarySize;
            if (dictionarySize < (uint)(1 << Base.kDicLogSizeMin) ||
                dictionarySize > (uint)(1 << kDicLogSizeMaxCompress))
                throw new ArgumentException(nameof(options.DictionarySize));
            ...
            var v = options.PosStateBits;
            if (v < 0 || v > (uint)Base.kNumPosStatesBitsEncodingMax)
                throw new ArgumentException(nameof(options.PosStateBits));
            _posStateBits = v;
            _posStateMask = (((uint)1) << _posStateBits) - 1;
            v = options.LitPosBits;
            if (v < 0 || v > Base.kNumLitPosStatesBitsEncodingMax)
                throw new ArgumentException(nameof(options.LitPosBits));
            _numLiteralPosStateBits = v;

            v = options.LitContextBits;
            if (v < 0 || v > Base.kNumLitContextBitsMax)
                throw new ArgumentException(nameof(options.LitContextBits));
            _numLiteralContextBits = v;

            SetWriteEndMarkerMode(options.EndMarker);
