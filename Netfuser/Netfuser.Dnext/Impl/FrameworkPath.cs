...
namespace Netfuser.Dnext.Impl
{
    ...
    sealed class FrameworkPaths : IComparable<FrameworkPaths>
    {
        ...
        static int CompareTo(string[] a, string[] b)
        {
            ...
            {
                int c = StringComparer.OrdinalIgnoreCase.Compare(Path.GetDirectoryName(a[i]),
                    Path.GetDirectoryName(b[i]));
                if (c != 0)
                    return c;
            }
    ...
    readonly struct FrameworkVersion : IComparable<FrameworkVersion>, IEquatable<FrameworkVersion>
    {
        ...
        public override int GetHashCode() =>
            Major ^ Minor ^ Patch ^ StringComparer.Ordinal.GetHashCode(Extra ?? string.Empty);
    }
}