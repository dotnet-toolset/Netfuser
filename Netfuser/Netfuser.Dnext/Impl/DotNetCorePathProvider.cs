...
{
    public sealed class DotNetCorePathProvider
    {
        ...
        readonly struct FrameworkVersionIgnoreExtra : IEquatable<FrameworkVersionIgnoreExtra>
        {
            ...
            public override int GetHashCode() =>
                version.Major ^ version.Minor ^ version.Patch ^ (version.Extra.Length == 0 ? 0 : -1);
        }
        ...
        public DotNetCorePathProvider()
        {
            ...
            var paths = from p in list
                group p by new
                {
                    Path = (Path.GetDirectoryName(Path.GetDirectoryName(p.Path)) ?? string.Empty).ToUpperInvariant(),
                    p.Bitness, Version = new FrameworkVersionIgnoreExtra(p.Version)
                }
                into g
        ...
        static IEnumerable<string> GetDotNetCoreBaseDirCandidates()
        {
            ...
            foreach (var envVar in envVars)
            {
                ...
                foreach (var path in pathEnvVar.Split(new[] {Path.PathSeparator},
            ...
            if (!string.IsNullOrEmpty(progDirX86) && StringComparer.OrdinalIgnoreCase.Equals(progDirX86, progDir) &&
                Path.GetDirectoryName(progDir) is string baseDir)
        ...
        /*static bool TryGetInstallLocationFromRegistry(string regPath, out string installLocation) {
        ...
        static IEnumerable<FrameworkPath> GetDotNetCorePaths(string basePath, int bitness)
        {
            ...
            foreach (var versionsDir in GetDirectories(sharedDir))
            {
                foreach (var dir in GetDirectories(versionsDir))
                {
                    ...
                    {
                        var g = m.Groups;
                        yield return new FrameworkPath(dir, bitness,
                            ToFrameworkVersion(g[1].Value, g[2].Value, g[3].Value, string.Empty));
                    }
                    else
                    {
                        ...
                        if (m.Groups.Count == 5)
                        {
                            var g = m.Groups;
                            yield return new FrameworkPath(dir, bitness,
                                ToFrameworkVersion(g[1].Value, g[2].Value, g[3].Value, g[4].Value));
                        }
                    }
                }
            }
